...
**A parallel investigation into replacing Koodo/Reader's client-side reading engine entirely is underway, separate from anything above** - see [`docs/kavita-fork-investigation.md`](./docs/kavita-fork-investigation.md). The motivation: Koodo's download-then-parse-client-side pipeline (the same one `ImportLocal` registers, described above) is a structural fail vector, not something patchable within this architecture. [Kavita](https://github.com/Kareadita/Kavita) is a real server (ASP.NET Core + Angular) that keeps books server-side and only sends the client rendered content, which removes that class of failure at the architecture level. As of the most recent spike (2026-09-25, on a dedicated Proxmox VM, not this repo's Cloudflare stack): Cloudflare Access can successfully act as Kavita's OIDC identity provider, confirmed end-to-end with a real login - but only after a one-line patch to Kavita's own source (`GetClaimsFromUserInfoEndpoint` is hardcoded `true` and breaks against Access's UserInfo response; see the doc for the full diagnosis and exact fix). No decision to actually fork has been made yet - this is still research, and nothing in this repo's own architecture (described above) changes because of it.
