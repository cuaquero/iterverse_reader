...
**Koodo/Reader's client-side reading engine is being replaced by a real fork, separate from anything above** - see [`docs/kavita-fork-investigation.md`](./docs/kavita-fork-investigation.md). The motivation: Koodo's download-then-parse-client-side pipeline (the same one `ImportLocal` registers, described above) is a structural fail vector, not something patchable within this architecture. [Kavita](https://github.com/Kareadita/Kavita) is a real server (ASP.NET Core + Angular) that keeps books server-side and only sends the client rendered content, which removes that class of failure at the architecture level. **As of 2026-09-26, this is no longer just research:** the fork ([cuaquero/iterverse_library](https://github.com/cuaquero/iterverse_library), rebranded "Iterverse Library") is decided, rebranded, and deployed at `library.iterverse.net` on the same Proxmox VM the original spike used (`192.168.200.21`) - auth goes through `kavita-oidc-bridge` (now living in `iterverse_library/kavita-oidc-bridge/`, not here), a standalone Cloudflare Worker that fronts Kavita's OIDC `authority` with Cloudflare Access plus a real Iterverse roster-entitlement check. This repo (Reader/Koodo) is expected to be retired once Library fully replaces it - nothing in this repo's own architecture (described above) has changed yet, but don't assume this repo has an indefinite future.
